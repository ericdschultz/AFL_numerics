...
def typ_pshear(N, k):
    """
    Returns the typical perturbation used for the quantum cat map of dimension N.
    This is the shear along p of k/2π * cos(2πq)
        generated by H(q) = -1/(2π)^2 * sin(2πq).
    """
    H = lambda q: -np.sin(2*np.pi*q) / (2*np.pi)**2
    return shear(N, k, H, 0, None)

def symbreak_qshear(N, k):
    """
    Returns a perturbation that breaks the cat W and R symmetries.
    We use a shear along q of k/4π * cos(4πp)
        generated by H(p) = 1/(4π)^2 * sin(4πp).
    N is the Hilbert space dimension.
    """
    H = lambda p: np.sin(4*np.pi*p) / (4*np.pi)**2
    return shear(N, 0, None, k, H)
